(function() {
    tinymce.PluginManager.add('wps_scg', function( editor, url ) {
        var tlpsc_tag = 'service';


        //add popup
        editor.addCommand('wps_scg_popup', function(ui, v) {
            //setup defaults

            editor.windowManager.open( {
                title: 'Wp Service ShortCode',
                width: jQuery( window ).width() * 0.3,
                height: (jQuery( window ).height() - 36 - 50) * 0.1,
                id: 'wps-sc-dialog',
                body: [
                    {
                        type   : 'container',
                        html   : '<span class="rt_loading">Loading...</span>'
                    },
                ],
                onsubmit: function( e ) {

                    var shortcode_str;
                    var id = jQuery("#scid").val();
                    var title = jQuery( "#scid option:selected" ).text();
                    if(id && id != 'undefined'){
                        shortcode_str = '[' + tlpsc_tag;
                            shortcode_str += ' id="'+id+'" title="'+ title +'"';
                        shortcode_str += ']';
                    }
                    if(shortcode_str) {
                        editor.insertContent(shortcode_str);
                    }else{
                        alert('No short code selected');
                    }
                }
            });

            putScList();
        });

        //add button
        editor.addButton('wps_scg', {
            icon: 'wps-scg',
            tooltip: 'Wp Service Showcase',
            cmd: 'wps_scg_popup',
        });

        function putScList(){
                var dialogBody = jQuery( '#wps-sc-dialog-body' )
                jQuery.post( ajaxurl, {
                    action: 'wpsShortCodeList'
                }, function( response ) {
                    dialogBody.html(response);
                    console.log(response);
                });

        }

    });
})();